    ...
    try:
        ...
        while more:
            ...
            for event in seq: ## seq should only ever have one element, but loop to be safe
                ...
                #    print('\t(%s/%s)' % (nativateEMSName, nativeProbableCause))
                cause = 'Unknow'
                if nativeProbableCause.find('TEMP_') >= 0:
                    #print('\t(%s/%s)' % (nativateEMSName, nativeProbableCause))
                    ...
                    if node_selected:
                        ...
                        if neTime.find(today) >= 0:
                            ...
                            if neTime.find(tomonth) >= 0:
                                if neDate != neOldDate:
                                    print >> stream, Style.BRIGHT + Fore.YELLOW + '%d   \t%s \t%s \t%s' % (event_counter, neTime, cause.ljust(10), nodename)
                                    neOldDate = neDate
                                else:
                                    print >> stream, Style.BRIGHT + Fore.YELLOW + '%d   \t%s \t%s \t%s' % (event_counter, neOnlyTime, cause.ljust(10), nodename)
                            elif neTime.find(toyear) >= 0:
                                if neDate != neOldDate:
                                    print >> stream, Style.BRIGHT + Fore.GREEN + '%d   \t%s \t%s \t%s' % (event_counter, neTime, cause.ljust(10), nodename)
                                    neOldDate = neDate
                                else:
                                    print >> stream, Style.BRIGHT + Fore.GREEN + '%d   \t%s \t%s \t%s' % (event_counter, neOnlyTime, cause.ljust(10), nodename)
                            else:
                                break;
                                if neDate != neOldDate:

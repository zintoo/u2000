...
class Type:
    ...
    def variable(self):
        ...
        if type.kind() in already_Variable:
    ...
    def __apply_mapping(self, xxx_todo_changeme, thing):
        ...
        # and a pointer types
        (const, ref, ptr) = xxx_todo_changeme
        text = thing
    ...
    def sequenceTemplate(self, environment = None, gscope = 0):
        ...
        if d_SeqType.char():
            ...
            template["forward"] = 1
        elif d_SeqType.type().kind() in typeSizeAlignMap:
    ...
    def __templateToString(self, template):
        ...
        if "forward" in template:
            name = name + "_Forward"

        elif "objref" in template and not template["array"]:
            name = name + "_ObjRef"

        elif "value" in template and not template["array"]:
            name = name + "_Value"
...
basic_map_out = { }
for key,value in list(basic_map.items()):
